...
wire mirror verbatim + its reachable CAS objects, `/c/index.json`, and
`/data/catalog/catalog.json` (the catalog-grid view-model, CONTRACTS.md §8)
-- under `--out`. `render-deploy.yml` (ADR-3, WP3-A) runs `site:build`
first, then `indexbot render --out` second, into the same
`emptyOutDir`-wiped tree (see `taskfile.yml`'s `render:build`).
